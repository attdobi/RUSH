      ...
      ${compact ? '' : `<div class="override-controls">
        <label>SME/human override
          <select class="override-label" data-sample-id="${attr(row.sample_id)}">
            ${overrideOptions.map(value => `<option value="${attr(value)}" ${override.label === value ? 'selected' : ''}>${esc(value)}</option>`).join('')}
          </select>
        </label>
        <label>Review note
...
// HTML. For multiclass demos (mnist) rebuild them from demo.classes so the
// per-class filters actually match the sampled row labels. GenAI keeps its
// original chips untouched.
function rebuildGalleryFilterChips() {
  const demo = activeDemo();
  if (demo.id === 'genai') return;
