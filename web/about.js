    ...
    <section class="about-section">
      ...
      <table class="about-knob-table">
        ...
        <tbody>
          ...
          <tr><td>Corrupt labels ρ (+ flip pool)</td><td>0 · random</td><td><strong>Protocol A of the label-noise lab</strong> (section below): flips a seeded fraction ρ ∈ [0, 0.6] of the run's train + test golden labels <em>in memory</em> — the label store, adjudication queue, holdout, and fixed benchmark never see a flipped label, and the flipped ids are recorded on the run (<code>corrupt_seed</code>, <code>flipped_ids</code>) for the corrupted-vs-clean twin analysis. Flip pool <code>random</code>: uniform over the dev pool, applied before the k=0 baseline. <code>anchors</code>: only images the panel already misaligned on at k=0 (tier-1/2 style targets; applied after the baseline pass, so k=0 metrics are pre-corruption). The sim predicts silence-type flips erase learning signal (slower, lower convergence) while sabotage-type flips inject spurious anchors (descent in chaotic directions) — <code>sim/label-noise/notebooks/policy_convergence_poc.ipynb</code>. Corrupted runs carry a red chip on the Benchmarks tab.</td></tr>
