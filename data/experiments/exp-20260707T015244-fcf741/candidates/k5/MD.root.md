...
## Compact/lookalike stroke-program priority
For compact, thick, or faint glyphs, decide from the whole intended stroke
program, not from one isolated cue such as “there is a loop,” “there is a top
segment,” or “there is one vertical stroke.” A claimed cue must have the right
role and position in the digit, not merely the same local shape.

Canonical boundary checks:

- **2 over accidental 0/9 readings:** a rounded upper curve flowing into a
  descending diagonal and then into a lower horizontal or near-horizontal
  base/terminal is a 2, even if the upper curve nearly closes or thick pixels
  form a loop-like blob. Use 0/6/9 only when the closed loop is the dominant
  structure and the stroke does not continue into a 2-like lower base.
- **4 over 5 when the bar is mid-height:** a 4 has a mid-level crossbar joining a
  left slanted/vertical stroke to a straight right stem, usually with the right
  stem continuing below the bar and an open or angular upper wedge. Do not treat
  this mid-crossbar/right-stem arrangement as the top cap of a 5. A 5's flat bar
  sits at the top and is followed by a left stem and one lower right-facing
  bowl/hook.
- **5 over 1/7 when a lower hook or bowl is present:** a 1 is a single simple
  stroke with at most serifs, and a 7 is a top bar plus one descending diagonal
  with no lower bowl or hook. If a top cap is followed by a drop/stem and a
  curved lower hook or bowl, prefer 5 even when the glyph is narrow, angular, or
  compressed. Prefer 6 instead only when the stroke is a continuous curl that
  encloses a lower loop, not merely a 5-like bowl bending back toward the stem.
- **6/9 over 0/1 when a free tail and loop placement are visible:** a 6 has a low
  loop plus an open upper curl/terminal; a 9 has a high loop plus a descending
  tail. A 0 has no free tail and is primarily a centered oval; a 1 has no
  rounded enclosed head. Do not collapse a 6 or 9 to 0 or 1 just because the tail
  is close to the loop, thin, or partially merged by rasterization.
- **8 over 9 when the lower part is a lobe:** below a central waist, a rounded
  lower portion that closes or near-closes as its own lobe is the second lobe of
  an 8, not a 9 tail. A 9's tail is a non-enclosing extension descending from a
  single upper loop.

Apply these priorities only to visible or inferable stroke structure; do not
invent missing bases, bowls, loops, or tails when the surrounding geometry does
not support them.
