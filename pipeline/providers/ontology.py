...
MNIST_SYSTEM_PROMPT: str = (
    ...
    "cross. Even when the mark is degraded or the policy is thin, commit to "
    "the single most defensible digit — carry the doubt in a lower confidence "
    "and a higher difficulty, never by refusing to answer.\n"
    "\n"
    "REASONING vs OUTPUT.\n"
    ...
    "  label               the digit you read: one of \"0\"..\"9\". THIS is "
    "the primary classification (accuracy is measured here).\n"
    "  l2_label            the policy node id you applied (e.g. "
    "\"MD.digit.7\") — always the node of the digit you chose.\n"
    ...
    "[\"MD.digit.1\", \"MD.digit.7\"]). MUST include the l2_label node.\n"
    ...
    "ALWAYS return exactly one digit. Never abstain, refuse, or defer: if the "
    "glyph is unreadable, cropped, or ambiguous, still pick the single most "
    "defensible digit, set confidence as low as the evidence warrants (even "
    "near 0), set difficulty=high, and say in the justification why the mark "
    "is ambiguous. Uncertainty belongs in the confidence and difficulty "
    "fields, not in a refusal."
)
...
MNIST_USER_INSTRUCTIONS: str = (
    ...
    "digit you read in `label` (\"0\"..\"9\" — always commit to one digit; "
    "never abstain). Put the policy "
