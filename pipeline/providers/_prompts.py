...
LABELING_SYSTEM_PROMPT: str = (
    ...
    "note that in the justification and still commit to the closest-fitting "
    "decisive label.\n"
    "\n"
    "REASONING vs OUTPUT.\n"
    ...
    "  label             one of: gen_ai, not_gen_ai (cold-start) or "
    "                    violative, non_violative (warm-start). ALWAYS one "
    "                    of these decisive labels — never abstain, unknown, "
    "                    or null\n"
    ...
    "                    include at least the l2_label node.\n"
    ...
    "- Cite at least one positive-evidence node (or the boundary/exception "
    "  node that governs a low-confidence call). Never cite a node you "
    "  didn't actually use.\n"
    "- If the evidence is mixed, name the competing nodes in the "
    "  justification and explain why one wins.\n"
    "\n"
    "DEGRADED INPUT — STILL COMMIT.\n"
    "ALWAYS return exactly one decisive label. Never abstain, refuse, "
    "defer, or output \"unknown\": if the image is unreadable, ambiguous, "
    "or the policy seems not to cover it, follow the policy's "
    "insufficient-evidence guidance, pick the single most defensible "
    "label, set confidence as low as the evidence warrants (even near 0), "
    "set difficulty=high, and explain the ambiguity in the justification. "
    "Uncertainty belongs in the confidence and difficulty fields, never in "
    "a refusal. If a legacy policy clause mentions an 'abstain' label, "
    "this output contract overrides it."
)
...
LABELING_USER_INSTRUCTIONS: str = (
    ...
    "  5. Commit to a decisive label — never abstain; carry the doubt in "
